...
func (s *PublicBlockChainAPI) GetExtra(ctx context.Context, address common.Address, blockNrOrHash rpc.BlockNumberOrHash) (hexutil.Bytes, error) {
	state, _, err := s.b.StateAndHeaderByNumberOrHash(ctx, blockNrOrHash)
	if state == nil || err != nil {
		return nil, err
	}
	extra := state.GetExtra(address)
	return hexutil.Bytes(extra), state.Error()
}
